...
//(function (window) {
	...
	function Calendar(el) {
		...
		this.calendarDays.forEach(function(d, pos) {
			...
			//var child = self.content.insertBefore($(d).children()[0], self.content.childNodes[0]);
			
			var child = $(d).children()[0];
			$( ".content" ).prepend( $(d).children()[0] );
			contents[ Number(pos) ] = child;
			var content = child;
	...
	Calendar.prototype._init = function(calendarEl) {
		calendarDays = [].slice.call(calendarEl.children);
		this.settings = {
			snow: true,
			tilt: false
		};
		bgEl = document.querySelector('.background');
